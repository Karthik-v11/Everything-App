...
/// The on-device model behind the assistant's interface (Phase 13).
///
/// ## What earns a test here, and what deliberately does not
///
/// Not the model. `flutter_gemma`'s statics cannot be faked, a 584 MB download
/// cannot run in CI, and asserting that a 1B model writes a good sentence is not
/// a test — it is a spike, and the plan says to run that on a device.
///
/// What earns a test is the **boundary**, which is where every decision in this
...
void main() {
  ...
  group('ModelBackedAiRepository.answerQuestion — grounded', () {
    test('the model only ever sees what search returned', () async {
      final gemma = _RecordingGemma(response: 'You have two tasks about rent.');
      final repository = ModelBackedAiRepository(
        delegate: _FakeRuleBasedAi(),
        engine: gemma,
        documentsRepository: _FakeDocuments(),
      ...
      // supply itself.
      expect(gemma.lastPrompt, contains('Pay rent'));
      expect(gemma.lastPrompt, contains('Renew passport'));
      expect(gemma.lastPrompt, contains('what about rent?'));
      ...
      // invented one.
      expect(gemma.lastSystemInstruction, contains('ONLY'));
    });
    ...
    test('an unloaded model falls back and never searches for nothing',
      ...
      final repository = ModelBackedAiRepository(
        delegate: _FakeRuleBasedAi(),
        engine: _ExplodingGemma(isLoaded: false),
        documentsRepository: _FakeDocuments(),
      );
    ...
    test('a failed generation falls back to the rule-based answer', () async {
      final repository = ModelBackedAiRepository(
        delegate: _FakeRuleBasedAi(),
        engine: _RecordingGemma(isSuccess: false),
        documentsRepository: _FakeDocuments(),
      );
    ...
    test('a blank generation falls back rather than answering with nothing',
      ...
      final repository = ModelBackedAiRepository(
        delegate: _FakeRuleBasedAi(),
        engine: _RecordingGemma(response: '   \n  '),
        documentsRepository: _FakeDocuments(),
      );
  ...
  group('ModelBackedAiRepository.summarizeDocument', () {
    test('the model is given the document and summarises it', () async {
      final gemma = _RecordingGemma(response: 'A note about the roof.');
      final repository = ModelBackedAiRepository(
        delegate: _FakeRuleBasedAi(),
        engine: gemma,
        documentsRepository: _FakeDocuments(),
      ...
      expect(gemma.lastPrompt, contains('The roof leaks'));
    });
...
/// [_ExplodingGemma] fails the test if it is generated from.
///
...
class _ExplodingGemma implements GemmaRepository {
  _ExplodingGemma({this.isLoaded = true});
  ...
  Future<JsonResponse> download({
    required void Function(int percent) onProgress,
      ...
      JsonResponse.created(message: 'ok');

  @override
  Future<JsonResponse> load() async => JsonResponse.success(message: 'ok');

  @override
  Future<void> unload() async {}

  @override
  Future<JsonResponse> remove() async => JsonResponse.success(message: 'ok');

  @override
  bool get isConfigured => true;

  @override
  int get downloadSizeBytes => 584 * 1000 * 1000;
}

/// [_RecordingGemma] answers, and remembers what it was asked — which is how the
/// grounding assertions read the prompt the model would actually have seen.
class _RecordingGemma extends _ExplodingGemma {
  _RecordingGemma({this.response = 'generated', this.isSuccess = true});
